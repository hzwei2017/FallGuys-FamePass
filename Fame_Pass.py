...
# 2023-12-15 | v5.1 | 8、多进程模式改为多线程模式以降低资源占用
# 2023-12-22 | v5.2 | 9、由于糖豆人将每周专题获得的经验降为20xp，所以该脚本失去意义了
# 2024-01-30 | v5.3 | 10、糖豆人恢复每周专题经验值
# 2024-01-31 | v5.4 | 11、加入季票等级平均执行效率变量，并监控该变量，在异常时发送邮件提醒
# 2024-02-22 | v5.5 | 12、修复监控脚本执行效率的bug；双倍经验日开始（2024-03-05 20:00结束）
# 2024-02-22 | v5.6 | 13、删除邮件装饰器和func4
# 2024-03-05 | v5.7 | 14、暂停func3；适配生存休闲模式；双倍经验日结束
# 2024-03-13 | v5.8 | 15、适配混合生存赛；更改func1中经验值计算方式
# 2024-03-22 | v5.9 | 16、适配重力蜂窝迷环试炼
# 2024-04-18 | v6.0 | 17、双倍经验日开始（2024-04-30 20:00结束）
# 
# ======README======
...
def func1():
    ...
    round_experience = 215 *2 #每局获得经验值，双倍经验日开始（2024-04-30 20:00结束）
...
def func2():
    # 录入原始x1, y1, rgb1, x2, y2, rgb2
    x1 = [60, 1560, 323, 880, 300, 483, 918, 20, 484, 257, 520, 1000, 40]
    y1 = [900, 1040, 18, 450, 320, 444, 388, 60, 235, 765, 48, 510, 40]
    rgb1 = [
        (49, 49, 49), (49, 49, 49), (34, 34, 38), (50, 201, 235), 
        (49, 202, 236), (38, 198, 236), (255, 255, 255), (34, 34, 38), 
        (119, 211, 238), (255, 255, 255), (255, 208, 68), (0, 182, 254), 
        (254, 106, 0)
    ]
    ...
    rgb2 = [
        (49, 49, 49), (49, 49, 49), (50, 201, 235), (255, 255, 255), 
        (255, 255, 255), (42, 176, 209), (188, 0, 130), (49, 49, 49), 
        (255, 255, 255), (0, 70, 236), (48, 194, 227), (49, 49, 49), 
        (49, 49, 49)
    ]
    # 变化坐标
    x1_offset = calc_x_offset(x1)
    ...
    while True:
        if is_window_active('FallGuys_client'):
            ...
            or pyautogui.pixelMatchesColor(x1_offset[11], y1_offset[11], rgb1[11], tolerance=2) and pyautogui.pixelMatchesColor(x2_offset[11], y2_offset[11], rgb2[11], tolerance=2) # 出局啦横幅
                ...
                # pyautogui.keyUp('a') # 结束func1的pyautogui.keyDown('a')
                pyautogui.press('esc')
                time.sleep(0.5)
            elif(
            ...
            or pyautogui.pixelMatchesColor(x1_offset[12], y1_offset[12], rgb1[12], tolerance=2) and pyautogui.pixelMatchesColor(x2_offset[12], y2_offset[12], rgb2[12], tolerance=2)    # 夺冠动画
                ):
